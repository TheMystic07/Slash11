namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        private string _walletAddress = ""; public string walletAddress { get { return data.walletAddress; } }
        private double _balance = 0; public double balance { get { return data.balanceSol; } }
        private long _balanceLamports = 0; public long balanceLamports { get { return data.balanceLamports; } }

        // Store last battle opponent data for access after battle
        [HideInInspector] public Data.Player lastBattleData = null;
        ...
        public void SendSyncRequest()
        {
            ...
            // Add wallet address to sync data if available
            bool hasWallet = !string.IsNullOrEmpty(data.walletAddress);
            p.Write(hasWallet);
            
            if (hasWallet) 
            {
                p.Write(data.walletAddress);
                p.Write(data.balanceSol);
                p.Write(data.balanceLamports);
                Debug.Log("Sending wallet data in sync: " + data.walletAddress + 
                         " with " + data.balanceSol + " SOL (" + data.balanceLamports + " lamports)");
            }
            else
            {
                Debug.Log("No wallet address available to send with sync");
            }
        ...
        {
            // Keep a copy of the previous balance to check if it changed
            double previousSolBalance = player.balanceSol;
            long previousLamports = player.balanceLamports;
            
            data = player;

            // If wallet address isn't set yet, register player with ApiClient
            if (string.IsNullOrEmpty(player.walletAddress))
            {
                // Check if we have a stored wallet address for this player
                string walletKey = "WALLET_ADDRESS_" + data.id;
                
                if (PlayerPrefs.HasKey(walletKey))
                {
                    // Use stored wallet address
                    data.walletAddress = PlayerPrefs.GetString(walletKey);
                    Debug.Log("Using stored wallet address: " + data.walletAddress);
                }
                else
                {
                    // Generate a consistent wallet address based on player ID
                    data.walletAddress = "testWallet" + data.id.ToString();
                    Debug.Log("Created new wallet address: " + player .walletAddress);
                    
                    // Store wallet address for future use
                    PlayerPrefs.SetString(walletKey, player.walletAddress);
                    PlayerPrefs.Save();
                    
                    // No longer setting initial balance locally - will fetch from API
                    // Instead of:
                    // data.balanceLamports = 300000000;
                    // data.balanceSol = (double)data.balanceLamports / Data.lamportsPerSol;
                    
                    // Just store the wallet address reference
                    _walletAddress = data.walletAddress;
                    
                    // Send REGISTER_PLAYER packet with account ID and device ID
                    Debug.Log("Registering new player: ID=" + data.id + ", Wallet=" + data.walletAddress);
                    Packet packet = new Packet();
                    packet.Write((int)RequestsID.REGISTER_PLAYER);
                    packet.Write(data.id.ToString()); // Send account ID
                    packet.Write(SystemInfo.deviceUniqueIdentifier); // Send device ID
                    packet.Write(data.walletAddress); // For reference only, API client will work with account ID
                    Sender.TCP_Send(packet);
                    
                    // Force an immediate sync to get the balance from server
                    timer = 0;
                }
            }
            
            // Check if we received a balance from the server in the player data
            if (data.balanceLamports > 0)
            {
                // Server provided a balance, use it directly
                Debug.Log("Using server-provided SOL balance: " + player.balanceSol + " SOL (" + data.balanceLamports + " lamports)");
                
                // Update stored balance to match server
                PlayerPrefs.SetFloat("SOL_BALANCE_" + data.id, (float)data.balanceSol);
                PlayerPrefs.Save();
            }
            else if (data.balanceLamports <= 0 && !string.IsNullOrEmpty(data.walletAddress))
            {
                // Server didn't provide a balance, use our local mock implementation
                Debug.Log("Server provided no SOL balance. Using local mock balance.");
                var balanceResult = GetMockSolanaBalance(data.walletAddress);
                data.balanceSol = balanceResult.Item1;
                data.balanceLamports = balanceResult.Item2;
            }
            
            // Log balance change
            if (previousSolBalance != data.balanceSol)
            {
                Debug.Log("SOL balance changed: " + previousSolBalance + " -> " + data.balanceSol + 
                         " (" + previousLamports + " -> " + data.balanceLamports + " lamports)");
            }
            
            // For gold, use the SOL balance in lamports
            _gold = data.balanceLamports > int.MaxValue ? int.MaxValue : (int)data.balanceLamports;
            _maxGold = int.MaxValue; // Set to max value for UI purposes
            ...
            */
            ...
            if (UI_Main.instanse.isActive && !UI_WarLayout.instanse.isActive && !UI_Scout.instanse.isActive)
            {
                ...
                if (UI_Main.instanse._grid.unidentifiedBuildings != null && UI_Main.instanse._grid.unidentifiedBuildings.Count > 0)
                {
                    ...
                    {
                        // For unplaced buildings, no need to deduct gold as we have "infinite" gold
        ...
        {
            // Display SOL amount in gold section
            UI_Main.instanse._goldText.text = string.Format("{0:0.000}", data.balanceSol) + " SOL";
            ...
            // Keep UI fill intact for consistency but use a fixed value for gold
            UI_Main.instanse._goldBar.fillAmount = 0.75f; // Fixed fill amount for SOL
        ...
        // Local class to avoid Server namespace dependency
        ...
        // Method to get balance through server API 
        // Rather than mocking locally, we now force a sync with the server
        private (double, long) GetMockSolanaBalance(string walletAddress)
        {
            Debug.Log("Requesting balance from server for " + walletAddress);
            
            // Send an immediate sync request to get the latest balance from the API
            SendSyncRequest();
            
            // Return current balance values, which will be updated when the server responds
            return (data.balanceSol, data.balanceLamports);
        }

        public void DeductSolForBattleSearch(int lamportsCost)
        {
            if (data.balanceLamports >= lamportsCost)
            {
                data.balanceLamports -= lamportsCost;
                data.balanceSol = (double)data.balanceSol;
                
                // Update stored balance
                PlayerPrefs.SetFloat("SOL_BALANCE_" + data.id, (float)data.balanceSol);
                PlayerPrefs.Save();
                
                // Update gold amount shown in UI since it's tied to SOL
                _gold = data.balanceLamports > int.MaxValue ? int.MaxValue : (int)data.balanceLamports;
                UpdateResourcesUI();
                
                Debug.Log($"Deducted {lamportsCost} lamports for battle search. New balance: {data.balanceSol} SOL");
                
                // Send packet to deduct SOL on server (asynchronously)
                double solAmount = (double)lamportsCost / Data.lamportsPerSol;
                Packet packet = new Packet();
                packet.Write((int)RequestsID.DEDUCT_SOL);
                packet.Write(Player.instanse.data.id.ToString());  // Send account ID directly
                packet.Write(solAmount);
                Sender.TCP_Send(packet);
            }
        }

        // Add this method to be called after a battle ends to call the attack endpoint
        private void CallAttackEndpoint(double damagePercent)
        {
            if (!string.IsNullOrEmpty(Player.instanse.data.walletAddress) &&
                Player.instanse.lastBattleData != null &&
                !string.IsNullOrEmpty(Player.instanse.lastBattleData.walletAddress))
            {
                Debug.Log("Sending BATTLE_ATTACK packet with attacker and defender account IDs");
                
                Packet packet = new Packet();
                packet.Write((int)Player.RequestsID.BATTLE_ATTACK);
                packet.Write(Player.instanse.data.id.ToString());  // Send attacker account ID
                packet.Write(Player.instanse.lastBattleData.id.ToString());  // Send defender account ID
                packet.Write(damagePercent);
                Sender.TCP_Send(packet);
                
                Debug.Log($"Battle attack call sent with {damagePercent}% damage");
            }
            else
            {
                Debug.LogWarning("Missing wallet information, cannot send battle attack data");
            }
        }
