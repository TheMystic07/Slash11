...
namespace DevelopersHub.RealtimeNetworking.Server
{
    class Terminal
    {
            ...
            try
            {
                ...
                switch ((RequestsID)id)
                {
                    ...
                    case RequestsID.RENAME:
                        ...
                        break;
                    case RequestsID.BATTLE_ATTACK:
                        string accountId1 = packet.ReadString();
                        string accountId2 = packet.ReadString();
                        double damagePercent = packet.ReadDouble();
                        
                        // Extract account IDs from wallet addresses
                        string attackerId = accountId1;
                        string defenderId = accountId2;
                      
                        
                        ProcessAttack(attackerId, defenderId, damagePercent);
                        break;
                    case RequestsID.DEDUCT_SOL:
                        string walletAddress = packet.ReadString();
                        double solAmount = packet.ReadDouble();
                        
                        // Extract account ID from wallet address
                        string accountIdForDeduct = walletAddress;
                       
                        
                        DeductSol(accountIdForDeduct, solAmount);
                        break;
                    case RequestsID.REGISTER_PLAYER:
                        string accountId = packet.ReadString();
                        string deviceId = packet.ReadString();
                        string newWalletAddress = packet.ReadString();
                        RegisterPlayer(accountId, deviceId, newWalletAddress);
                        break;
                }
            }
            catch (Exception ex)
        ...
        // Helper method to process attack via API
        private static void ProcessAttack(string attackerId, string defenderId, double damagePercent)
        {
            try
            {
                ApiClient.ProcessAttack(attackerId, defenderId, damagePercent);
                Console.WriteLine($"Processing attack: Attacker={attackerId}, Defender={defenderId}, Damage={damagePercent}%");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error processing attack: {ex.Message}");
            }
        }

        // Helper method to deduct SOL via API
        private static void DeductSol(string accountId, double amount)
        {
            try
            {
                ApiClient.DeductSolana(accountId, amount);
                Console.WriteLine($"Deducting {amount} SOL from account: {accountId}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error deducting SOL: {ex.Message}");
            }
        }

        // Helper method to register player via API
        private static void RegisterPlayer(string accountId, string deviceId, string walletAddress)
        {
            try
            {
                Console.WriteLine($"Registering player: Account ID={accountId}, Device ID={deviceId}, Wallet={walletAddress}");
                
                // Call the API client to register the player
                var response = ApiClient.RegisterPlayer(accountId, deviceId);
                
                if (response != null && response.Success)
                {
                    Console.WriteLine($"Player registered successfully with wallet: {response.Address}");
                ...
                {
                    Console.WriteLine("Failed to register player with API");
            ...
            {
                Console.WriteLine($"Error registering player: {ex.Message}");
            }
        }
        
        // Helper method to update wallet address in database
        private static void UpdateWalletInDatabase(long accountId, string walletAddress)
        {
            try
            {
