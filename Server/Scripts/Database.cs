    ...
    class Database
    {
            ...
            using (MySqlConnection connection = GetMysqlConnection())
            {
                ...
                if (found == false)
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        ApiClient.RegisterPlayer(initializationData.accountID.ToString(), device);
                    }
                    ...
                    AddResources(connection, initializationData.accountID, 1000000000, 10000, 0, 250);
                }
        ...
        public async static void SyncPlayerData(int id, string device)
        {
            ...
            if (player != null)
            {
                ...
                player.walletAddress = ApiClient.GetWalletAddress(account_id.ToString());                
                if (!string.IsNullOrEmpty(player.walletAddress))
                {
                    try
                    {
                        // Extract account ID from wallet address if needed
                        string accountId = player.id.ToString();
                        if (player.walletAddress.StartsWith("testWallet"))
                        {
                            accountId = player.walletAddress.Substring("testWallet".Length);
                        }
                        
                        var (sol, lamports) = ApiClient.GetSolanaBalance(accountId);
                        player.balanceSol = sol;
                        player.balanceLamports = lamports;
                        Console.WriteLine($"Player {account_id} has {sol} SOL ({lamports} lamports)");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Failed to get SOL balance for player {account_id}: {ex.Message}");
                    }
                }
        ...
        private static Data.Player _GetPlayerDataAsync(long id)
        {
            ...
            using (MySqlConnection connection = GetMysqlConnection())
            {
                string query = String.Format("SELECT id, name, gems, trophies, banned, shield, level, xp, clan_join_timer, clan_id, clan_rank, war_id, NOW() AS now_time, email, map_layout, shld_cldn_1, shld_cldn_2, shld_cldn_3, wallet_address FROM accounts WHERE id = {0};", id);
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                data.walletAddress = reader["wallet_address"].ToString();
                                
                                // If wallet address exists, attempt to get Solana balance
                                if (!string.IsNullOrEmpty(data.walletAddress))
                                {
                                    try {
                                        // Extract account ID from wallet address if needed
                                        string accountId = data.id.ToString();
                                        if (data.walletAddress.StartsWith("testWallet"))
                                        {
                                            accountId = data.walletAddress.Substring("testWallet".Length);
                                        }
                                        
                                        var (sol, lamports) = ApiClient.GetSolanaBalance(accountId);
                                        data.balanceSol = sol;
                                        data.balanceLamports = lamports;
                                    } catch {
                                        // Default values already set in Player class initialization
                                    }
                                }
                            }
                        }
