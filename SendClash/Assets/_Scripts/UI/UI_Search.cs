namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class UI_Search : MonoBehaviour
    {
        ...
        public void Find()
        {
            ...
            // Get battle search cost in lamports
            int townHallLevel = 1;
            ...
            int costInLamports = Data.GetBattleSearchCost(townHallLevel);
            
            // Deduct SOL from player balance
            Player.instanse.DeductSolForBattleSearch(costInLamports/1000000000);
            
            // Send battle find request to server
        ...
        private void Check()
        {
            ...
            // Display battle search cost in SOL
            double solCost = (double)cost / Data.lamportsPerSol;
            _costText.text = string.Format("{0:0.0000}", solCost) + " SOL";
            
            // Check if player has enough SOL
            if (cost > Player.instanse.data.balanceLamports)
            {
                _findButton.interactable = false;
