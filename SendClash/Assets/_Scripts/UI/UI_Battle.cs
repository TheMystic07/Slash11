namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class UI_Battle : MonoBehaviour
    {
        ...
        // Store defender's SOL balance
        private double defenderSolBalance = 0;
        ...
        public bool Display(Data.Player player, List<Data.Building> buildings, long defender, Data.BattleType battleType)
        {
            ...
            // Store defender's SOL balance
            defenderSolBalance = player.balanceSol;
            ...
            for (int i = 0; i < buildings.Count; i++)
            {
                ...
                switch (building.building.id)
                {
                    case Data.BuildingID.townhall:
                        // Use 25% of defender's SOL balance for townhall gold storage
                        double solLoot = defenderSolBalance * 0.25;
                        building.lootGoldStorage = (int)(solLoot * Data.lamportsPerSol);
                    ...
                    case Data.BuildingID.goldstorage:
                        // Use 25% of defender's SOL balance for gold storage
                        double solStorageLoot = defenderSolBalance * 0.25;
                        building.lootGoldStorage = (int)(solStorageLoot * Data.lamportsPerSol);
                        break;
        ...
        private void UpdateLoots()
        {
            ...
            // Display SOL values instead of gold
            double solLooted = (looted.Item4 - looted.Item1) / (double)Data.lamportsPerSol;
            _lootGoldText.text = string.Format("{0:0.000}", solLooted) + " SOL";
        ...
        public void BattleEnded(int stars, int unitsDeployed, int lootedGold, int lootedElixir, int lootedDark, int trophies, int frame)
        {
            ...
            // Display SOL values instead of gold
            double solLooted = lootedGold / (double)Data.lamportsPerSol;
            _endGoldText.text = string.Format("{0:0.000}", solLooted) + " SOL";
        ...
        public void EndBattle(bool surrender, int surrenderFrame)
        {
            ...
            // Send attack information to API with damage percentage
            if (target > 0 && Player.instanse.data.id > 0)
            {
                // Create a new packet to send the attack data
                Packet attackPacket = new Packet();
                attackPacket.Write((int)Player.RequestsID.BATTLE_ATTACK);
                attackPacket.Write(Player.instanse.data.id.ToString());
                attackPacket.Write(target.ToString());
                attackPacket.Write(battle.percentage);
                Sender.TCP_Send(attackPacket);
            }
        }
