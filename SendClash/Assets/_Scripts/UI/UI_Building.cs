    ...
    public class UI_Building : MonoBehaviour
    {
        ...
        public void Initialize(bool haveWorker)
        {
            ...
            if(building != null)
            {
                ...
                {
                    double solCost = (double)building.requiredGold / Data.lamportsPerSol*1000;
                    _resourceText.text = string.Format("{0:0.00000000}", solCost);
                ...
                if (building.requiredGold <= Player.instanse.data.balanceLamports && building.requiredElixir <= Player.instanse.elixir && building.requiredDarkElixir <= Player.instanse.darkElixir && building.requiredGems <= Player.instanse.data.gems)
                {
