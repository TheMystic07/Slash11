...
namespace DevelopersHub.RealtimeNetworking.Client
{
    public class Packet : IDisposable
    {
        public enum ID
        {
            INITIALIZATION = 1, NULL = 2, CUSTOM = 3, STRING = 4, BOOLEAN = 5, FLOAT = 6, LONG = 7, SHORT = 8, VECTOR3 = 9, QUATERNION = 10, BYTE = 11, BYTES = 12, INTEGER = 13, DECIMAL = 14
        }
        ...
        /// <param name="_value">The decimal to add.</param>
        public void Write(decimal _value)
        {
            buffer.AddRange(BitConverter.GetBytes((double)_value));
        }
        #endregion
        ...
        /// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
        public decimal ReadDecimal(bool _moveReadPos = true)
        {
            try
            {
                if (buffer.Count > readPos)
                {
                    decimal _value = (decimal)BitConverter.ToDouble(readableBuffer, readPos);
                    if (_moveReadPos)
                    {
                        readPos += 8;
                    }
                ...
                {
                    throw new Exception("Could not read value of type 'decimal'!");
                }
            }
            catch
            {
