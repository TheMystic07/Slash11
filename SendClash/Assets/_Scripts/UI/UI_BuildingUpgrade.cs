    ...
    public class UI_BuildingUpgrade : MonoBehaviour
    {
        ...
        public void Open()
        {
            ...
            if (x >= 0)
            {
                ...
                if(serverBuilding != null && !(Player.instanse.data.buildings[x].id == Data.BuildingID.townhall && Player.instanse.data.buildings[x].level >= Data.maxTownHallLevel))
                {
                    ...
                    if (Player.instanse.data.balanceLamports < serverBuilding.requiredGold || 
                        serverBuilding.requiredElixir > Player.instanse.elixir || 
                        serverBuilding.requiredDarkElixir > Player.instanse.darkElixir || 
                        serverBuilding.requiredGems > Player.instanse.data.gems)
                    {
                        haveResources = false;
                    }
                    ...
                    double solCost = (double)serverBuilding.requiredGold / Data.lamportsPerSol;
                    reqGold.text = string.Format("{0:0.00000}", solCost) + " SOL";
        ...
        private void Upgrade()
        {
            if(serverBuilding != null)
            {
                ...
                if (serverBuilding.requiredGold > 0)
                {
                    selectedInstanse.DeductSolForPurchase(serverBuilding.requiredGold * Data.lamportsPerSol);
                }
