namespace DevelopersHub.ClashOfWhatecer
{
    ...
    public class UI_Shop : MonoBehaviour
    {
        ...
        public void SetStatus(bool status)
        {
            if (status)
            {
                double solBalance = (double)Player.instanse.data.balanceLamports / Data.lamportsPerSol;
                _goldText.text = string.Format("{0:0.0000}", solBalance) + " SOL";
        ...
        public bool PlaceBuilding(Data.BuildingID id, int x = -1, int y = -1)
        {
            ...
            if (prefab.Item1 != null)
            {
                ...
                for (int i = 0; i < Player.instanse.initializationData.serverBuildings.Count; i++)
                {
                    ...
                    sbi = i;
                    havrResources = Player.instanse.data.gems >= Player.instanse.initializationData.serverBuildings[i].requiredGems && 
                                   Player.instanse.elixir >= Player.instanse.initializationData.serverBuildings[i].requiredElixir && 
                                   Player.instanse.data.balanceLamports >= Player.instanse.initializationData.serverBuildings[i].requiredGold && 
                                   Player.instanse.darkElixir >= Player.instanse.initializationData.serverBuildings[i].requiredDarkElixir;
                    break;
                }
