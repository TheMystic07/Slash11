...
/*
        ...
        // Add this method for SOL deduction when purchasing buildings or upgrades
        public void DeductSolForPurchase(double goldCost)
        {
            // // Use account ID instead of wallet address
            // if (goldCost > 0)
            Debug.Log("[building] SOL for gold cost: " + goldCost);
                double solAmount = goldCost / Data.lamportsPerSol * 1000;
                Debug.Log("[building]Deducting SOL for gold cost: " + solAmount);
                
                // Send deduction request to server via packet with account ID
                Packet packet = new Packet();
                packet.Write((int)Player.RequestsID.DEDUCT_SOL);
                packet.Write(Player.instanse.data.id.ToString());  // Send account ID directly
                packet.Write(solAmount);
                Sender.TCP_Send(packet);
                
                Debug.Log($"Sent SOL deduction request for {solAmount} SOL from account ID {Player.instanse.data.id}");
        
        }

        // Method to adjust level UI elements
        public void AdjustLevel()
        {
            // This replaces the SetCurrentLevel function that was missing
            CheckLevel();
        }
